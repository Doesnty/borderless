    ...
    while (gBattleStruct->turnEffectsBattlerId < gBattlersCount && gBattleStruct->turnEffectsTracker <= ENDTURN_BATTLER_COUNT)
    {
        ...
        else
        {
            ...
            case ENDTURN_INGRAIN:  // ingrain
                ...
                break;
            case ENDTURN_AQUARING:
                if (gDisableStructs[gActiveBattler].aquaRingSet 
                 && gBattleMons[gActiveBattler].hp != gBattleMons[gActiveBattler].maxHP
                 && gBattleMons[gActiveBattler].hp != 0)
                {
                    gBattleMoveDamage = gBattleMons[gActiveBattler].maxHP / 16;
            ...
            case ENDTURN_TAUNT:  // taunt
                if (gDisableStructs[gActiveBattler].tauntTimer)
				{
					...
					if (!gDisableStructs[gActiveBattler].tauntTimer)
					{
                        ...
                        BattleScriptExecute(BattleScript_TauntWearsOff);
                        ++effect;
					}
				}
                ++gBattleStruct->turnEffectsTracker;
			...
			case ENDTURN_BAD_DREAMS:
				if (gBattleMons[gActiveBattler ^ 0x1].ability == ABILITY_BAD_DREAMS && (gBattleMons[gActiveBattler ^ 0x1].hp > 0) &&
				(gBattleMons[gActiveBattler].status1 & STATUS1_SLEEP) && gBattleMons[gActiveBattler].hp > 0
					&& gBattleMons[gActiveBattler].ability != ABILITY_MAGIC_GUARD)
				{
                    gBattleMoveDamage = gBattleMons[gActiveBattler].maxHP / 8;
                    if (gBattleMoveDamage == 0)
			...
			case ENDTURN_ROOST:
				if (gBattleMons[gActiveBattler].type1 == TYPE_ROOSTING)
					gBattleMons[gActiveBattler].type1 = TYPE_WIND;
				if (gBattleMons[gActiveBattler].type2 == TYPE_ROOSTING)
					gBattleMons[gActiveBattler].type2 = TYPE_WIND;
                ++gBattleStruct->turnEffectsTracker;
				break;
        ...
        case CANCELLER_FLINCH: // flinch
            if (gBattleMons[gBattlerAttacker].status2 & STATUS2_FLINCHED)
            {
                ...
                if (gBattleMons[gBattlerAttacker].ability == ABILITY_STEADFAST &&
                    gBattleMons[gBattlerAttacker].statStages[STAT_SPEED] < 0xC)
                {
...
u8 AbilityBattleEffects(u8 caseID, u8 battler, u8 ability, u8 special, u16 moveArg)
{
    ...
    if (!(gBattleTypeFlags & BATTLE_TYPE_SAFARI)) // Why isn't that check done at the beginning?
    {
        ...
        case ABILITYEFFECT_ON_SWITCHIN: // 0
            ...
            case ABILITYEFFECT_SWITCH_IN_WEATHER:
                ...
                case WEATHER_DOWNPOUR:
                    if (!(gBattleWeather & WEATHER_RAIN_ANY))
                    {
                        ...
                        gWishFutureKnock.weatherDuration = 120;
                        ++effect;
                    }
                    break;
            ...
            case ABILITY_RECOLLECTION:
            case ABILITY_LOOSE_LIPS:
            ...
            case ABILITY_HISOUTEN:
                ...
                {
                    gStatuses3[battler] |= STATUS3_TRACE;
                    BattleScriptPushCursorAndCallback(BattleScript_CloudNineAnnounces);
                    gBattleScripting.battler = battler;
                    ++effect;
                }
                break;
            case ABILITY_PRESSURE:
                ...
                {
                    gStatuses3[battler] |= STATUS3_TRACE;
                    BattleScriptPushCursorAndCallback(BattleScript_PressureAnnounces);
                    gBattleScripting.battler = battler;
                    ++effect;
                }
                break;
            ...
            case ABILITY_MOLD_BREAKER:
                ...
                {
                    gStatuses3[battler] |= STATUS3_TRACE;
                    BattleScriptPushCursorAndCallback(BattleScript_MoldBreakerAnnounces);
                    gBattleScripting.battler = battler;
                    ++effect;
                }
                break;
            case ABILITY_PURE_FURIES:
                if (!(gStatuses3[battler] & STATUS3_TRACE))
                {
                    gStatuses3[battler] |= STATUS3_TRACE;
                    BattleScriptPushCursorAndCallback(BattleScript_PureFuriesAnnounces);
                    gBattleScripting.battler = battler;
                    ++effect;
                }
                break;
            ...
            case ABILITY_LUNACY:
                ...
                {
                    u8 rn = 1 + Random() % 7;
                    gBattleMons[battler].statStages[rn]++;
                    gBattleScripting.animArg1 = 0x0E + rn;
                    gBattleScripting.animArg2 = 0;
                    
                    
                    PREPARE_STAT_BUFFER(gBattleTextBuff1, rn)
                    gStatuses3[battler] |= STATUS3_TRACE;
                    BattleScriptPushCursorAndCallback(BattleScript_LunacyActivates);
                    gBattleScripting.battler = battler;
                    ++effect;
                }
                break;
            case ABILITY_POWER_LEAK:
                if (!(gStatuses3[battler] & STATUS3_TRACE))
                {
                    ...
                    gBattleScripting.animArg1 = 0x12;
                    gBattleScripting.animArg2 = 0;
                    
                    
                    PREPARE_STAT_BUFFER(gBattleTextBuff1, STAT_SPATK)
                    gStatuses3[battler] |= STATUS3_TRACE;
                    BattleScriptPushCursorAndCallback(BattleScript_PowerLeakBoost);
                    gBattleScripting.battler = battler;
                    ++effect;
                }
                break;
        ...
        case ABILITYEFFECT_ENDTURN: // 1
            if (gBattleMons[battler].hp != 0)
            {
                ...
                case ABILITY_SPEED_BOOST:
                    ...
                    {
						if (gBattleMons[battler ^ 0x1].ability == ABILITY_STASIS_GAZE && gBattleMons[battler ^ 0x1].hp > 0)
						{
							BattleScriptPushCursorAndCallback(BattleScript_StasisGazePreventsAbility);
							gBattleScripting.battler = battler;
							++effect;
						}
						else
                    ...
                    break;
                case ABILITY_POWER_LEAK:
                    if (gBattleMons[battler].statStages[STAT_SPATK] > 0 && gDisableStructs[battler].isFirstTurn != 2)
                    {
						if (gBattleMons[battler ^ 0x1].ability == ABILITY_STASIS_GAZE && gBattleMons[battler ^ 0x1].hp > 0)
						{
							BattleScriptPushCursorAndCallback(BattleScript_StasisGazePreventsAbility);
						...
						{
							gBattleMons[battler].statStages[STAT_SPATK]--;
							gBattleScripting.animArg1 = STAT_ANIM_MINUS1 + STAT_SPATK - 1;
							gBattleScripting.animArg2 = 0;
							BattleScriptPushCursorAndCallback(BattleScript_PowerLeakPenalty);
							gBattleScripting.battler = battler;
							++effect;
						}
                    }
				...
				case ABILITY_HARVEST:
					...
					{
						u16 berry = gBattleStruct->usedHeldItems[gBattlerPartyIndexes[battler] + (GetBattlerSide(battler) ? 0 : 6)];
						if (gBattleMons[battler].item == 0 && GetPocketByItemId(berry) == POCKET_BERRY_POUCH)
						{
							gActiveBattler = battler;
        ...
        case ABILITYEFFECT_MOVE_END: // Think contact abilities.
			...
			case ABILITY_LAST_GRUDGE:
                 ...
                 && TARGET_TURN_DAMAGED)
                {
					...
					if (i != MAX_MON_MOVES && gBattleMons[gBattlerAttacker].pp[i] > 1)
					{
						PREPARE_MOVE_BUFFER(gBattleTextBuff1, gCurrentMove)
						gBattleMons[gBattlerAttacker].pp[i] = 1;
						gActiveBattler = gBattlerAttacker;
            ...
            case ABILITY_CONTAGION:
                 ...
                 && gBattleMons[gBattlerAttacker].ability != ABILITY_CONTAGION)
                {
                    gBattleMons[gBattlerAttacker].ability = ABILITY_CONTAGION;
                    BattleScriptPushCursor();
                    gBattlescriptCurrInstr = BattleScript_ContagionSpreads;
                    ++effect;
                }
                break;
            case ABILITY_LECTURE:
                 ...
                 && TARGET_TURN_DAMAGED)
                {
                    if (gBattleMons[gBattlerAttacker].ability == ABILITY_CONTRARY &&
                        gBattleMons[gBattlerAttacker].statStages[STAT_SPEED] < 12)
                    {
                        gBattleMons[gBattlerAttacker].statStages[STAT_SPEED]++;
						// we have to use the "wrong" animation arg here
						// since it'll get flipped later
                        gBattleScripting.animArg1 = GET_STAT_BUFF_ID(STAT_SPEED) + STAT_ANIM_MINUS1 - 1;
                        gBattleScripting.animArg2 = 0;
                        BattleScriptPushCursor();
                        gBattlescriptCurrInstr = BattleScript_LectureContrary;
                        gEffectBattler = gBattlerTarget;
                        ++effect;
                    }
                    else if (gBattleMons[gBattlerAttacker].ability == ABILITY_HAKUREI_MIKO ||
                             gBattleMons[gBattlerAttacker].ability == ABILITY_MAGIC_BARRIER ||
                             gBattleMons[gBattlerAttacker].ability == ABILITY_SOUNDPROOF)
                    {
                        BattleScriptPushCursor();
                        gBattlescriptCurrInstr = BattleScript_LectureBlockedByAbility;
                        gEffectBattler = gBattlerTarget;
                        ++effect;
                    }
                    else
                    {
                        ...
                        gBattleScripting.animArg2 = 0;
                        BattleScriptPushCursor();
                        gBattlescriptCurrInstr = BattleScript_Lecture;
                        gEffectBattler = gBattlerTarget;
                        ++effect;
                    }
                }
			...
			case ABILITY_BUZZER:
                if (gBattleMons[gBattlerAttacker].hp != 0
                 && gBattleMons[gBattlerTarget].hp != 0
				 && ((gMoveResultFlags & MOVE_RESULT_NOT_VERY_EFFECTIVE) || (gMoveResultFlags & MOVE_RESULT_DOESNT_AFFECT_FOE))
                 && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
                 && !gSideTimers[GET_BATTLER_SIDE(gBattlerAttacker)].mistTimer)
				 {
					 if (gBattleMoves[gCurrentMove].moveClass == CLASS_PHYSICAL && gBattleMons[gBattlerAttacker].statStages[STAT_ATK] > 0)
					 {
						if (gBattleMons[gBattlerAttacker].ability == ABILITY_CONTRARY &&
							gBattleMons[gBattlerAttacker].statStages[STAT_ATK] < 12)
						{
							...
							gBattleScripting.animArg1 = GET_STAT_BUFF_ID(STAT_ATK) + STAT_ANIM_MINUS1 - 1;
							gBattleScripting.animArg2 = 0;
							BattleScriptPushCursor();
							...
							++effect;
								 ...
								 gBattleMons[gBattlerAttacker].ability == ABILITY_MAGIC_BARRIER ||
								 gBattleMons[gBattlerAttacker].ability == ABILITY_HI_STRENGTH ||
								 gBattleMons[gBattlerAttacker].ability == ABILITY_SOUNDPROOF)
						{
							BattleScriptPushCursor();
							gBattlescriptCurrInstr = BattleScript_LectureBlockedByAbility;
					 ...
					 else if (gBattleMoves[gCurrentMove].moveClass == CLASS_SPECIAL && gBattleMons[gBattlerAttacker].statStages[STAT_SPATK] > 0)
					 {
							...
							gBattleMons[gBattlerAttacker].statStages[STAT_SPATK] < 12)
						{
							...
							gBattleScripting.animArg1 = GET_STAT_BUFF_ID(STAT_SPATK) + STAT_ANIM_MINUS1 - 1;
							gBattleScripting.animArg2 = 0;
							BattleScriptPushCursor();
							gBattlescriptCurrInstr = BattleScript_BuzzerContrary;
							gEffectBattler = gBattlerTarget;
            ...
            case ABILITY_RATTLED:
                if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
                 ...
                 && TARGET_TURN_DAMAGED)
                {
					if (gBattleMons[gBattlerAttacker].ability == ABILITY_STASIS_GAZE && gBattleMons[gBattlerAttacker].hp > 0)
					{
						BattleScriptPushCursorAndCallback(BattleScript_StasisGazePreventsAbility);
						gBattleScripting.battler = battler;
						++effect;
            ...
            case ABILITY_REACTIVE:
                 ...
                 && TARGET_TURN_DAMAGED)
                {
                    ...
                    else if (gBattleMoves[gCurrentMove].moveClass == CLASS_PHYSICAL
			...
			case ABILITY_CURSED_BODY:
				 ...
				 && TARGET_TURN_DAMAGED)
				{
					...
					if (i != 4 && gBattleMons[gBattlerAttacker].pp[i] != 0)
					{
						PREPARE_MOVE_BUFFER(gBattleTextBuff1, gCurrentMove)
						...
						BattleScriptPushCursor();
						gBattlescriptCurrInstr = BattleScript_CursedBody;
						++effect;
					}
				}
				break;
			case ABILITY_JUSTIFIED:
				if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
				 && gBattleMons[gBattlerTarget].hp != 0
				 && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
				 && (gBattleMoves[gCurrentMove].type == TYPE_DARK || gBattleStruct->dynamicMoveType == TYPE_DARK)
				 && gBattleMons[gBattlerTarget].statStages[STAT_ATK] < 12
				 && TARGET_TURN_DAMAGED)
				{
					if (gBattleMons[gBattlerAttacker].ability == ABILITY_STASIS_GAZE && gBattleMons[gBattlerAttacker].hp > 0)
			...
			case ABILITY_BERSERK:
				if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
				 ...
				 && gBattleMons[gBattlerTarget].statStages[STAT_SPATK] < 12
				 && TARGET_TURN_DAMAGED)
				{
					if (gBattleMons[gBattlerAttacker].ability == ABILITY_STASIS_GAZE && gBattleMons[gBattlerAttacker].hp > 0)
					{
            ...
            for (i = 0; i < gBattlersCount; ++i)
            {
                if (gBattleMons[i].ability == ABILITY_TRACE && (gStatuses3[i] & STATUS3_TRACE))
                {
                    ...
                    if (effect)
                    {
                        ...
                        if (gLastUsedAbility != ABILITY_TRACE)
                        {
                ...
                else if (gBattleMons[i].ability == ABILITY_RECOLLECTION && (gStatuses3[i] & STATUS3_TRACE)
                        && gBattleMons[i].hp == gBattleMons[i].maxHP)
                {
					...
					if (gBattleMons[target1].hp != 0 && !(gBattleMons[target1].status2 & STATUS2_SUBSTITUTE) && !(gBattleMons[target1].status2 & STATUS2_TRANSFORMED))
					{
                        ...
                        gBattleMons[i].status2 |= STATUS2_TRANSFORMED;
						for (j = 0; j < 4; j++)
						{
						...
						gBattleScripting.animArg1 = i;
						gBattleScripting.animArg2 = target1;
                ...
                else if (gBattleMons[i].ability == ABILITY_LOOSE_LIPS && (gStatuses3[i] & STATUS3_TRACE)
                        && gBattleMons[i].hp == gBattleMons[i].maxHP)
                {
                    target1 = GetBattlerPosition(i) ^ 1;
                    gStatuses3[i] &= ~(STATUS3_TRACE);
                    if (gBattleMons[target1].hp != 0 && !(gBattleMons[target1].status2 & STATUS2_SUBSTITUTE))
                    {
						gBattlerTarget = target1;
						BattleScriptPushCursorAndCallback(BattleScript_LooseLips);
						gBattleScripting.battler = i;
                        gBattleMoveDamage = 1;
						effect++;
						break;
				...
				else if (gBattleMons[i].ability == ABILITY_DOWNLOAD && (gStatuses3[i] & STATUS3_TRACE))
				{
					...
					{
						def += ( gBattleMons[target1].defense * gStatStageRatios[gBattleMons[target1].statStages[STAT_DEF]][0])
								/ gStatStageRatios[gBattleMons[target1].statStages[STAT_DEF]][1];
						spdef += ( gBattleMons[target1].spDefense * gStatStageRatios[gBattleMons[target1].statStages[STAT_SPDEF]][0])
								/ gStatStageRatios[gBattleMons[target1].statStages[STAT_SPDEF]][1];
					}
					
					if (gBattleMons[target2].hp != 0)
					{
						def += ( gBattleMons[target2].defense * gStatStageRatios[gBattleMons[target2].statStages[STAT_DEF]][0])
								/ gStatStageRatios[gBattleMons[target2].statStages[STAT_DEF]][1];
						spdef += ( gBattleMons[target2].spDefense * gStatStageRatios[gBattleMons[target2].statStages[STAT_SPDEF]][0])
								/ gStatStageRatios[gBattleMons[target2].statStages[STAT_SPDEF]][1];
					}
					
					if (def < spdef && gBattleMons[i].statStages[STAT_ATK] < 12)
					{
						if (gBattleMons[i ^ 0x1].ability == ABILITY_STASIS_GAZE)
						{
							BattleScriptPushCursorAndCallback(BattleScript_StasisGazePreventsAbility);
							gBattleScripting.battler = battler;
							++effect;
						...
						else
						{
							...
							gBattleScripting.animArg1 = GET_STAT_BUFF_ID(STAT_ATK) + STAT_ANIM_PLUS1 - 1;
							gBattleScripting.animArg2 = 0;
					...
					else if (def >= spdef && gBattleMons[i].statStages[STAT_SPATK] < 12)
					{
						if (gBattleMons[i ^ 0x1].ability == ABILITY_STASIS_GAZE)
						{
							BattleScriptPushCursorAndCallback(BattleScript_StasisGazePreventsAbility);
						...
						else
						{
							...
							gBattleScripting.animArg1 = GET_STAT_BUFF_ID(STAT_SPATK) + STAT_ANIM_PLUS1 - 1;
							gBattleScripting.animArg2 = 0;
					...
					if (gBattleMons[target1].hp != 0)
					{
						for (j = 0; j < 4; j++)
						{
							...
							if (gBattleMoves[move].effect == EFFECT_OHKO)
								power = 160;
							if (move == MOVE_COUNTER || move == MOVE_MIRROR_COAT)
								power = 160;
        ...
        case ABILITYEFFECT_COUNT_ON_FIELD: // 18
            ...
            break;
        case ABILITYEFFECT_ECHO:
            switch (gLastUsedAbility)
            {
                case ABILITY_MANIC_ECHO: // raise own attacking stat
                {
                    u8 stat = STAT_ATK;
					if (gBattleMons[battler ^ 0x1].ability == ABILITY_STASIS_GAZE && gBattleMons[battler ^ 0x1].hp > 0)
					{
					...
					else
					{
							...
							&& !(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE))
						{
							...
							gBattleScripting.animArg1 = 0x0E + stat;
							gBattleScripting.animArg2 = 0;
							
							PREPARE_STAT_BUFFER(gBattleTextBuff1, stat)
							BattleScriptPushCursorAndCallback(BattleScript_ManicEchoBoost);
							gBattleScripting.battler = battler;
							effect++;
						}
					}
                ...
                {
                    u8 stat = STAT_ATK;
                    gBattlerTarget = GetBattlerPosition(battler) ^ 1;
                        ...
                        && gBattleMons[gBattlerTarget].hp != 0)
                    {
                        ...
                        {
                            // blocked by mist
                            BattleScriptPushCursorAndCallback(BattleScript_DourEchoBlockedByMist);
                            gBattleScripting.battler = battler;
                            effect++;
                            
                        }
                        else if ((gBattleMons[gBattlerTarget].ability == ABILITY_HI_STRENGTH && stat == STAT_ATK) ||
                                 gBattleMons[gBattlerTarget].ability == ABILITY_MAGIC_BARRIER ||
                                 gBattleMons[gBattlerTarget].ability == ABILITY_HAKUREI_MIKO ||
                                 gBattleMons[gBattlerTarget].ability == ABILITY_SOUNDPROOF)
                        {
                            // blocked by ability
                            BattleScriptPushCursorAndCallback(BattleScript_DourEchoBlockedByAbility);
                            gBattleScripting.battler = battler;
                            effect++;
                        }
                        else
                        {
                            ...
                            PREPARE_STAT_BUFFER(gBattleTextBuff1, stat)
                            BattleScriptPushCursorAndCallback(BattleScript_DourEchoPenalty);
                            gBattleScripting.battler = battler;
                            effect++;
                        }
                    }
                ...
                case ABILITY_PRIDE:
                {
                    if ((gBattleMoves[gCurrentMove].effect == EFFECT_RESTORE_HP
                     || gBattleMoves[gCurrentMove].effect == EFFECT_REST
                     || gBattleMoves[gCurrentMove].effect == EFFECT_SYNTHESIS
                     || gBattleMoves[gCurrentMove].effect == EFFECT_HEAL_BELL
                     || gBattleMoves[gCurrentMove].effect == EFFECT_REFRESH
                     || gBattleMoves[gCurrentMove].effect == EFFECT_WISH
                     || gBattleMoves[gCurrentMove].effect == EFFECT_HEALING_WISH
                     || gBattleMoves[gCurrentMove].effect == EFFECT_INGRAIN
                     || gBattleMoves[gCurrentMove].effect == EFFECT_AQUA_RING
                     || gBattleMoves[gCurrentMove].effect == EFFECT_AEGIS_MERGE)
                     && !(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE)
                     ...
                     && gBattleMons[battler].statStages[STAT_SPATK] < 12)
                    {
						...
						else
						{
							...
							PREPARE_STAT_BUFFER(gBattleTextBuff1, STAT_SPATK);
							BattleScriptPushCursorAndCallback(BattleScript_PrideBoost);
							gBattleScripting.battler = battler;
							++effect;
						}
                    }
...
u8 ItemBattleEffects(u8 caseID, u8 battlerId, bool8 moveTurn)
{
    ...
    if (gBattleMons[battlerId].ability == ABILITY_GOURMAND)
    {
		gluttonyMult = 8;
		gluttonyFigyMult = 2;
	}
	...
	stasisGaze = (gBattleMons[gBattlerTarget ^ 0x1].ability == ABILITY_STASIS_GAZE && gBattleMons[gBattlerTarget ^ 0x1].hp > 0);
    ...
    case 1:
        if (gBattleMons[battlerId].hp)
        {
            ...
            case HOLD_EFFECT_CONFUSE_SPICY:
                if (gluttonyFigyMult * gBattleMons[battlerId].hp <= gBattleMons[battlerId].maxHP / 4 && !moveTurn)
                {
                    ...
                    if (GetFlavorRelationByPersonality(gBattleMons[battlerId].personality, FLAVOR_SPICY) < 0
                        || (gBattleTypeFlags & BATTLE_TYPE_IMAKUNI && battlerId == 1))
            ...
            case HOLD_EFFECT_ATTACK_UP:
                if (!stasisGaze && gBattleMons[battlerId].hp <= gluttonyMult * gBattleMons[battlerId].maxHP / battlerHoldEffectParam && !moveTurn && gBattleMons[battlerId].statStages[STAT_ATK] < 0xC)
                {
                    PREPARE_STAT_BUFFER(gBattleTextBuff1, STAT_ATK);
            ...
            case HOLD_EFFECT_DEFENSE_UP:
                if (!stasisGaze && gBattleMons[battlerId].hp <= gluttonyMult * gBattleMons[battlerId].maxHP / battlerHoldEffectParam && !moveTurn && gBattleMons[battlerId].statStages[STAT_DEF] < 0xC)
                {
            ...
            case HOLD_EFFECT_SPEED_UP:
                if (!stasisGaze && gBattleMons[battlerId].hp <= gluttonyMult * gBattleMons[battlerId].maxHP / battlerHoldEffectParam && !moveTurn && gBattleMons[battlerId].statStages[STAT_SPEED] < 0xC)
                {
            ...
            case HOLD_EFFECT_SP_ATTACK_UP:
                if (!stasisGaze && gBattleMons[battlerId].hp <= gluttonyMult * gBattleMons[battlerId].maxHP / battlerHoldEffectParam && !moveTurn && gBattleMons[battlerId].statStages[STAT_SPATK] < 0xC)
                {
                    PREPARE_STAT_BUFFER(gBattleTextBuff1, STAT_SPATK);
                    gEffectBattler = battlerId;
            ...
            case HOLD_EFFECT_SP_DEFENSE_UP:
                if (!stasisGaze && gBattleMons[battlerId].hp <= gluttonyMult * gBattleMons[battlerId].maxHP / battlerHoldEffectParam && !moveTurn && gBattleMons[battlerId].statStages[STAT_SPDEF] < 0xC)
                {
            ...
            case HOLD_EFFECT_RANDOM_STAT_UP:
                if (!stasisGaze && !moveTurn && gBattleMons[battlerId].hp <= gluttonyMult * gBattleMons[battlerId].maxHP / battlerHoldEffectParam)
                {
