...
static const u8 sMoldBreakerAbilities[] =
{
	...
	ABILITY_INSOMNIA,
	ABILITY_LIMBER,
	ABILITY_FIRE_VEIL, // redundant but may become relevant?
	ABILITY_OBLIVIOUS,
	ABILITY_OWN_TEMPO,
	ABILITY_SOUNDPROOF,
	ABILITY_GATE_KEEPER,
...
static void atk01_accuracycheck(void)
{
    ...
    else
    {
		...
		if (atkHoldEffect == HOLD_EFFECT_WIDE_LENS)
			calc = (calc * 110) / 100; // 1.1 wide lens boost
...
static void atk06_typecalc(void)
{
    ...
    {
        gLastUsedAbility = defenderAbility;
	...
	if (gMoveResultFlags & MOVE_RESULT_SUPER_EFFECTIVE && gBattleMoveDamage > 0 && defenderAbility == ABILITY_FILTER)
	{
		gBattleMoveDamage *= 3;
		gBattleMoveDamage /= 4;
	}
...
static void ModulateDmgByType2(u8 multiplier, u16 move, u16 *flags)
{
    if (multiplier == 1) // watch for foresight, scrappy
    {
        if (gBattleMons[gBattlerTarget].status2 & STATUS2_FORESIGHT ||
    ...
    if (multiplier == 2) // watch for lost dreams
    {
        if (gBattleMons[gBattlerAttacker].ability == ABILITY_LOST_DREAM)
            multiplier = 10;
        else
            multiplier = 0;
    }
    if (multiplier == 3) // resistance possibly subverted by move
    {
        if (move == MOVE_FREEZE_DRY || move == MOVE_HEART_BREAK)
            multiplier = 20;
        else
            multiplier = 5;
    }
...
u8 TypeCalc(u16 move, u8 attacker, u8 defender)
{
    ...
    {
        u8 matchup = gTypeEffectiveness[moveType][gBattleMons[defender].type1];
        ModulateDmgByType2(matchup, move, &flags);
        if (gBattleMons[defender].type1 != gBattleMons[defender].type2)
        {
            matchup = gTypeEffectiveness[moveType][gBattleMons[defender].type2];
            ModulateDmgByType2(matchup, move, &flags);
        }
    }
    if (defenderAbility == ABILITY_PLAY_GHOST
...
u8 AI_TypeCalc(u16 move, u16 targetSpecies, u8 targetAbility)
{
    ...
    {
        u8 matchup = gTypeEffectiveness[moveType][type1];
        ModulateDmgByType2(matchup, move, &flags);
        if (type1 != type2)
        {
            matchup = gTypeEffectiveness[moveType][type2];
            ModulateDmgByType2(matchup, move, &flags);
        }
    }
...
static void atk07_adjustnormaldamage(void)
{
     ...
     && gBattleMons[gBattlerTarget].hp <= gBattleMoveDamage)
    {
        ...
        else if (defenderAbility == ABILITY_STURDY &&
        ...
        if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
        {
            ...
            else
            {
                ...
                else // hp goes down
                {
                    ...
                    if (gBattleMoves[gCurrentMove].moveClass == 0 && !(gHitMarker & HITMARKER_PASSIVE_DAMAGE) && gCurrentMove != MOVE_PAIN_SPLIT)
                    {
                    ...
                    else if (gBattleMoves[gCurrentMove].moveClass == 1 && !(gHitMarker & HITMARKER_PASSIVE_DAMAGE))
                    {
        ...
        else
        {
            ...
            default:
                ...
                else if (gMoveResultFlags & MOVE_RESULT_STURDY)
                {
                    gMoveResultFlags &= ~(MOVE_RESULT_STURDY);
                    BattleScriptPushCursor();
                    gBattlescriptCurrInstr = BattleScript_EnduredWithSturdyMsg;
                    return;
                }
...
void SetMoveEffect(bool8 primary, u8 certain)
{
    ...
    if (gBattleMons[gEffectBattler].ability == ABILITY_FLAWLESS 
    ...
    if (gBattleCommunication[MOVE_EFFECT_BYTE] <= 6) // status change
    {
        ...
        case STATUS1_PARALYSIS:
            ...
            if (gBattleMons[gEffectBattler].status1 || gBattleMons[gEffectBattler].type1 == TYPE_ELECTRIC ||
				gBattleMons[gEffectBattler].type2 == TYPE_ELECTRIC)
                break;
            statusChanged = TRUE;
            break;
...
static void atk20_jumpifstat(void)
{
    u8 ret = 0;
    u8 battlerId = GetBattlerForBattleScript(gBattlescriptCurrInstr[1]);
	u8 comparison = gBattlescriptCurrInstr[2];
    u8 value = gBattleMons[battlerId].statStages[gBattlescriptCurrInstr[3]];
	u8 contrary = gBattleMons[battlerId].ability == ABILITY_CONTRARY;
...
static void atk45_playanimation(void)
{
	...
	{
		switch (gBattleScripting.animArg1)
		{
            case 0xF ... 0x15: gBattleScripting.animArg1 += 7; break;
            case 0x16 ... 0x1C: gBattleScripting.animArg1 -= 7; break;
            case 0x27 ... 0x2D: gBattleScripting.animArg1 += 7; break;
            case 0x2E ... 0x34: gBattleScripting.animArg1 -= 7; break;
            
            case 0x37 ... 0x38: gBattleScripting.animArg1 += 2; break;
            case 0x39 ... 0x3A: gBattleScripting.animArg1 -= 2; break;
		}
	}
...
static void atk48_playstatchangeanimation(void)
{
	...
	arg3 = gBattlescriptCurrInstr[3];
	
	if (gBattleMons[gActiveBattler].ability == ABILITY_CONTRARY)
		arg3 ^= 0x1;
	
    if (arg3 & ATK48_STAT_NEGATIVE) // goes down
    {
        s16 startingStatAnimId;
        if (arg3 & ATK48_STAT_BY_TWO)
        ...
        while (statsToCheck != 0)
        {
            if (statsToCheck & 1)
            {
                if (arg3 & ATK48_DONT_CHECK_LOWER)
                {
    ...
    if (arg3 & ATK48_ONLY_MULTIPLE && changeableStatsCount < 2)
    {
    ...
    else if (changeableStatsCount != 0 && !gBattleScripting.statAnimPlayed)
    {
        ...
        MarkBattlerForControllerExec(gActiveBattler);
        if (arg3 & ATK48_ONLY_MULTIPLE && changeableStatsCount > 1)
    ...
    do
    {
		...
		case ATK49_MOXIE:
				...
				&& TARGET_TURN_DAMAGED)
			{
                ...
                gBattleScripting.battler = gBattlerAttacker;
				BattleScriptPushCursor();
				gBattlescriptCurrInstr = BattleScript_Devourer;
				effect = TRUE;
			}
			++gBattleScripting.atk49_state;
			break;
        ...
        case ATK49_WALL_MASTER_CLEANUP:
            ...
            ++gBattleScripting.atk49_state;
            break;
		case ATK49_TWIN_SPARK:
			if (gBattleMons[gBattlerAttacker].ability == ABILITY_TWIN_SPARK &&
                !(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE) &&
				(gBattleMoves[gCurrentMove].moveClass != CLASS_STATUS) &&
            ...
            ++gBattleScripting.atk49_state;
			break;
		case ATK49_TWIN_SPARK_CLEANUP:
            gBattleStruct->twinSparkTracker = 0;
            ++gBattleScripting.atk49_state;
            break;
...
static void atk4A_typecalc2(void)
{
    u8 flags = 0;
    s32 i = 0;
    s32 damagebuffer = gBattleMoveDamage;
    u8 moveType = gBattleMoves[gCurrentMove].type;
    
    atk06_typecalc();
    
    gBattleMoveDamage = damagebuffer;
}
...
static void atk52_switchineffects(void)
{
    ...
    if (!(gSideStatuses[GetBattlerSide(gActiveBattler)] & SIDE_STATUS_SPIKES_DAMAGED))
    {
        ...
        if (gBattlescriptCurrInstr[1] == BS_TARGET)
            ...
            gBattlescriptCurrInstr = BattleScript_HazardsAttacker;
        else
            gBattlescriptCurrInstr = BattleScript_HazardsFaintedBattler;
    }
    else
    {
...
static void atk5D_getmoneyreward(void)
{
    ...
    u32 moneyReward;
    u8 lastMonLevel = 0;
    u8 moneyMultiplier = 1;

    const struct TrainerMonItemCustomMoves *party4; //This needs to be out here

    if (gBattleStruct->moneyMultiplier & MONEY_MULTIPLIER_AMULET_COIN)
        moneyMultiplier *= 2;
    if (gBattleStruct->moneyMultiplier & MONEY_MULTIPLIER_HAPPY_HOUR)
        moneyMultiplier *= 2;
    ...
    if (gBattleOutcome == B_OUTCOME_WON)
    {
        ...
        else
        {
            ...
            moneyReward = 4 * lastMonLevel * moneyMultiplier * (gBattleTypeFlags & BATTLE_TYPE_DOUBLE ? 2 : 1) * gTrainerMoneyTable[i].value;
        }
...
static void atk6A_removeitem(void)
{
    ...
    gActiveBattler = GetBattlerForBattleScript(gBattlescriptCurrInstr[1]);
    usedHeldItem = &gBattleStruct->usedHeldItems[gBattlerPartyIndexes[gActiveBattler] + (GetBattlerSide(gActiveBattler) ? 0 : 6)];
...
static void atk7D_setrain(void)
{
    ...
    {
		u16 holdEffect = ItemId_GetHoldEffect(gBattleMons[gBattlerAttacker].item);
		if (gBattleMons[gBattlerAttacker].ability == ABILITY_KLUTZ)
			holdEffect = 0;
...
static void atk7E_setreflect(void)
{
    ...
    {
		u16 itemEffect = ItemId_GetHoldEffect(gBattleMons[gBattlerAttacker].item);
		if (gBattleMons[gBattlerAttacker].ability == ABILITY_KLUTZ)
			itemEffect = 0;
		
        gSideStatuses[GET_BATTLER_SIDE(gBattlerAttacker)] |= SIDE_STATUS_REFLECT;
		if (itemEffect == HOLD_EFFECT_LIGHT_CLAY)
...
static u8 ChangeStatBuffs(s8 statValue, u8 statId, u8 flags, const u8 *BS_ptr)
{
    ...
    if ((statValue <= -1 && !contrary) || (statValue >= 0 && contrary)) // Stat decrease.
    {
        ...
        else if (gBattleMons[gActiveBattler].ability == ABILITY_HI_STRENGTH
...
static void atk91_givepaydaymoney(void)
{
    ...
    if (gBattleStruct->moneyMultiplier & MONEY_MULTIPLIER_HAPPY_HOUR)
        moneyMultiplier *= 2;
    if (!(gBattleTypeFlags & BATTLE_TYPE_LINK) && (gPaydayMoney != 0 || gDebtSpiralMoney != 0))
    {
        s32 bonusMoney = (gPaydayMoney * moneyMultiplier) - gDebtSpiralMoney;
...
static void atk96_weatherdamage(void)
{
    ...
    if (WEATHER_HAS_EFFECT)
    {
        ...
        {
            if (gBattleMons[gBattlerAttacker].ability == ABILITY_SOLAR_POWER)
            {
...
{
    if (gBattleMons[gBattlerTarget].ability != ABILITY_NONE && gBattleMons[gBattlerTarget].ability != ABILITY_PLAY_GHOST)
    {
...
static void atkD4_trywish(void)
{
    ...
    case 0: // use wish
        if (gWishFutureKnock.wishCounter[gBattlerAttacker] == 0)
        {
			...
			gWishFutureKnock.wishStrength[gBattlerAttacker] = gBattleMons[gBattlerAttacker].maxHP / 2;
    ...
    case 1: // heal effect
        ...
        gBattleMoveDamage = gWishFutureKnock.wishStrength[gBattlerTarget];
        if (gBattleMoveDamage == 0)
            gBattleMoveDamage = 1;
        gBattleMoveDamage *= -1;
        if (gBattleMons[gBattlerTarget].hp == gBattleMons[gBattlerTarget].maxHP)
...
static void atkD5_trysetroots(void) // ingrain
{
    ...
    {
        if (gCurrentMove == MOVE_AEGIS_MERGE)
            gStatuses3[gBattlerAttacker] |= STATUS3_AEGIS_MERGE;
...
static void atkE5_pickup(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; ++i)
    {
		...
		// replace item if it was a trainer battle
		// if a berry turned into something else, just remove it
		// don't replace berries
		oldItem = gBattleStruct->itemsAtBattleStart[i];
		if (gBattleTypeFlags & BATTLE_TYPE_TRAINER && oldItem != heldItem)
		{
			if (GetPocketByItemId(oldItem) == POCKET_BERRY_POUCH)
				newItem = 0;
			else
				newItem = oldItem;
        ...
        if (species != SPECIES_NONE && species != SPECIES_EGG && heldItem == ITEM_NONE && ability != ABILITY_JEALOUSY)
        {
            ...
            if (!newItem && !(Random() % 10))
            {
				s32 random = Random() % 100;
				for (j = 0; TRUE; ++j)
					if (sSpeciesPickupTables[gBaseStats[species].touhou][j].chance > random)
						break;
				newItem = sSpeciesPickupTables[gBaseStats[species].touhou][j].itemId;
            }
            ...
            if (!newItem && !(Random() % 20) && species != SPECIES_CSHION && species != SPECIES_SHION)
            {
				...
				if (newItem == ITEM_OCCA_BERRY)
				{
					...
					switch(type)
					{
						...
						case TYPE_FIGHTING: newItem = ITEM_CHOPLE_BERRY; break;
						case TYPE_WIND: newItem = ITEM_COBA_BERRY; break;
						case TYPE_MIASMA: newItem = ITEM_KEBIA_BERRY; break;
						case TYPE_EARTH: newItem = ITEM_SHUCA_BERRY; break;
						case TYPE_BEAST: newItem = ITEM_CHARTI_BERRY; break;
						case TYPE_HEART: newItem = ITEM_TANGA_BERRY; break;
						case TYPE_GHOST: newItem = ITEM_KASIB_BERRY; break;
						case TYPE_STEEL: newItem = ITEM_BABIRI_BERRY; break;
						case TYPE_FIRE: newItem = ITEM_OCCA_BERRY; break;
						case TYPE_WATER: newItem = ITEM_PASSHO_BERRY; break;
						case TYPE_NATURE: newItem = ITEM_RINDO_BERRY; break;
						case TYPE_ELECTRIC: newItem = ITEM_WACAN_BERRY; break;
						case TYPE_REASON: newItem = ITEM_PAYAPA_BERRY; break;
						case TYPE_ICE: newItem = ITEM_YACHE_BERRY; break;
						case TYPE_FAITH: newItem = ITEM_HABAN_BERRY; break;
						case TYPE_DARK: newItem = ITEM_COLBUR_BERRY; break;
						default:
						case TYPE_ILLUSION: newItem = ITEM_ENIGMA_RESIST_BERRY; break;
					}
				}
            }
...
static void sp05_trickroom(void);
static void sp06_tailwind(void);
static void sp07_uturncheck(void);
static void sp08_purify(void);
static void sp09_healingwish(void);
static void sp0a_healingwishaffect(void);
static void sp0b_spikesaffect(void);
static void sp0c_acupressure(void);
static void sp0d_psychoshiftsleep(void);
static void sp0e_bestow(void);
...
static void sp24_set_last_wish_attacker(void);
static void sp25_last_wish_affect(void);
...
void (* const gBattleScriptingSpecialsTable[])(void) =
{
    ...
    sp08_purify,
    sp09_healingwish,
    sp0a_healingwishaffect,
    sp0b_spikesaffect,
    sp0c_acupressure,
    sp0d_psychoshiftsleep,
    sp0e_bestow,
    sp0f_happyhour,
    sp10_mefirst,
	...
	sp24_set_last_wish_attacker,
	sp25_last_wish_affect,
...
static void sp06_tailwind(void)
{
    if (gSideTimers[GET_BATTLER_SIDE(gBattlerAttacker)].tailwindTimer)
    {
		gBattlescriptCurrInstr = BattleScript_ButItFailed;
    }
    else
...
static void sp08_purify(void)
{
	if (gBattleMons[gBattlerTarget].type1 == TYPE_NORMAL && gBattleMons[gBattlerTarget].type2 == TYPE_NORMAL)
	{
...
static void sp09_healingwish(void)
{
	...
	MarkBattlerForControllerExec(gActiveBattler);
	gSideTimers[GetBattlerSide(gActiveBattler)].healingWish = 1;
}
...
static void sp0a_healingwishaffect(void)
{
	u8 wish;
	gActiveBattler = gBattleScripting.battler;
	wish = gSideTimers[GetBattlerSide(gActiveBattler)].healingWish;
	if (!wish)
	{
		gBattlescriptCurrInstr += 5;
	}
	...
	{
		gSideTimers[GetBattlerSide(gActiveBattler)].healingWish = 0;
		gBattleMoveDamage = gBattleMons[gActiveBattler].maxHP * (-1);
		gBattleMons[gActiveBattler].status1 = 0;
...
{	while (1)
	{
        ...
        case 0: 
            if (gBattleMons[gBattlerAttacker].statStages[STAT_ATK] < 12)
            {
                gBattleScripting.statChanger = 33;
                return;
            }
            break;
        case 1: 
            ...
            {
                gBattleScripting.statChanger = 34;
                return;
            }
            break;
        case 2: 
            ...
            {
                gBattleScripting.statChanger = 35;
                return;
            }
            break;
        case 3: 
            ...
            {
                gBattleScripting.statChanger = 36;
                return;
            }
            break;
        case 4: 
            ...
            {
                gBattleScripting.statChanger = 37;
                return;
            }
            break;
        case 5: 
            ...
            {
                gBattleScripting.statChanger = 38;
                return;
            }
            break;
        case 6: 
            ...
            {
                gBattleScripting.statChanger = 39;
                return;
            }
            break;
		}
...
static void sp0e_bestow(void)
{
    ...
    // took a while, but all checks passed and items can be safely swapped
    // code lifted from trick since it's functionally the same at this point
    else
    {
        u16 oldItemAtk, *newItemAtk;
...
static void sp0f_happyhour(void)
{
    ...
    {
        gBattleStruct->moneyMultiplier |= MONEY_MULTIPLIER_HAPPY_HOUR;
    }
}
...
static void sp10_mefirst(void)
{
	...
	if (gActionsByTurnOrder[gBattlerTarget] != B_ACTION_USE_MOVE)
		failure = 1;
	if (gProtectStructs[gBattlerTarget].noValidMoves)
	{
		move = MOVE_STRUGGLE;
		failure = 1;
	}
...
static void sp20_souvenir(void)
{
	 ...
	 && gLastPrintedMoves[gBattlerTarget] != MOVE_THIRD_EYE)
    {
		...
		gBattleMons[gBattlerAttacker].ppBonuses = 0;
		gActiveBattler = gBattlerAttacker;
		for (i = 0; i < MAX_MON_MOVES; ++i)
		{
			movePpData.moves[i] = gBattleMons[gBattlerAttacker].moves[i];
...
static void sp24_set_last_wish_attacker()
{
	u8 side;
	gActiveBattler = gBattlerAttacker;
	...
	if (gBattleMons[gActiveBattler].ability == ABILITY_LAST_WISH)
	{
        ...
        BattleScriptPushCursor();
        gBattlescriptCurrInstr = BattleScript_MadeLastWish;
	}
}
...
static void sp25_last_wish_affect(void)
{
	...
	gActiveBattler = gBattleScripting.battler;
	side = GetBattlerSide(gActiveBattler);
	wish = gSideTimers[side].lastWish;
	if (!wish)
	{
		gBattlescriptCurrInstr += 5;
	}
	...
	{
		gSideTimers[side].lastWish = 0;
		gBattleMoveDamage = gSideTimers[side].lastWishStrength;
		if (gBattleMoveDamage < 1)
			gBattleMoveDamage = 1;
		gBattleMoveDamage *= -1;
		//MarkBattlerForControllerExec(gActiveBattler);
...
static void sp26_set_last_wish_target()
{
	u8 side;
	gActiveBattler = gBattlerTarget;
	side = GetBattlerSide(gActiveBattler);
...
static void sp27_salvage_armor_bank0()
{
		...
		gBattleMons[0].statStages[STAT_DEF] < 12)
	{
		...
		gBattleMons[gBattleScripting.battler].statStages[STAT_DEF]++;
		gBattleScripting.animArg1 = 0x10;
		gBattleScripting.animArg2 = 0;
        BattleScriptPushCursor();
